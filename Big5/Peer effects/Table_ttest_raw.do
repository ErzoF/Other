...
drop estandar_Neuro_k estandar_Extrav_k estandar_Aperex_k estandar_Amab_k estandar_Resp_k estandar_Neuro estandar_Extrav estandar_Aperex estandar_Amab estandar_Resp

summarize
if r(N)!=0{
global tablas "C:\Users\Erzo\OneDrive - Universidad del Pacífico\Univesidad del Pacífico\Ciclo\lCiclo 7 2024 I\Asistente\Big5\Peer effects classrooms\Tablas"
export excel "$tablas/`cur'`sem'", firstrow(variable) replace
}
}
}
...
gen estandar_Resp_k=estandar_Resp if cachimbo==1
*/
collapse (first) acayearterm N SECCION_DEL_CURSO (first)NOMBRE_DEL_CURSO (mean)cachimbo Neuro Extrav Aperex Amab Resp estandar_Neuro estandar_Extrav estandar_Aperex estandar_Amab estandar_Resp, by (salon)
drop if N<10
...
keep if acayearterm=="`j'-I PER"
****
bys NOMBRE_DEL_CURSO: egen max_neuro_k_`j'1=max(Neuro_k)
bys NOMBRE_DEL_CURSO: egen min_neuro_k_`j'1=min(Neuro_k)
gen dummymax_neuro_k_`j'1=0
replace dummymax_neuro_k_`j'1=1 if max_neuro_k_`j'1==Neuro_k
gen dummymin_neuro_k_`j'1=0
replace dummymin_neuro_k_`j'1=1 if min_neuro_k_`j'1==Neuro_k
****
****
bys NOMBRE_DEL_CURSO: egen max_extrav_k_`j'1=max(Extrav_k)
bys NOMBRE_DEL_CURSO: egen min_extrav_k_`j'1=min(Extrav_k)
gen dummymax_extrav_k_`j'1=0
replace dummymax_extrav_k_`j'1=1 if max_extrav_k_`j'1==Extrav_k
gen dummymin_extrav_k_`j'1=0
replace dummymin_extrav_k_`j'1=1 if min_extrav_k_`j'1==Extrav_k
****
****
bys NOMBRE_DEL_CURSO: egen max_aperex_k_`j'1=max(Aperex_k)
bys NOMBRE_DEL_CURSO: egen min_aperex_k_`j'1=min(Aperex_k)
gen dummymax_aperex_k_`j'1=0
replace dummymax_aperex_k_`j'1=1 if max_aperex_k_`j'1==Aperex_k
gen dummymin_aperex_k_`j'1=0
replace dummymin_aperex_k_`j'1=1 if min_aperex_k_`j'1==Aperex_k
****
****
bys NOMBRE_DEL_CURSO: egen max_amab_k_`j'1=max(Amab_k)
bys NOMBRE_DEL_CURSO: egen min_amab_k_`j'1=min(Amab_k)
gen dummymax_amab_k_`j'1=0
replace dummymax_amab_k_`j'1=1 if max_amab_k_`j'1==Amab_k
gen dummymin_amab_k_`j'1=0
replace dummymin_amab_k_`j'1=1 if min_amab_k_`j'1==Amab_k
****
****
bys NOMBRE_DEL_CURSO: egen max_resp_k_`j'1=max(Resp_k)
bys NOMBRE_DEL_CURSO: egen min_resp_k_`j'1=min(Resp_k)
gen dummymax_resp_k_`j'1=0
replace dummymax_resp_k_`j'1=1 if max_resp_k_`j'1==Resp_k
gen dummymin_resp_k_`j'1=0
replace dummymin_resp_k_`j'1=1 if min_resp_k_`j'1==Resp_k

save "$bases/dummies_`j'1k", replace
}
...
merge m:1 salon acayearterm using "$bases/dummies_`i'k", nogen // keep(3)
if `i'==20201{
...
replace dummymax_neuro_k_`i'=0 if cachimbo==0
replace dummymin_neuro_k_`i'=0 if cachimbo==0
replace dummymax_extrav_k_`i'=0 if cachimbo==0
replace dummymin_extrav_k_`i'=0 if cachimbo==0
replace dummymax_aperex_k_`i'=0 if cachimbo==0
replace dummymin_aperex_k_`i'=0 if cachimbo==0
replace dummymax_amab_k_`i'=0 if cachimbo==0
replace dummymin_amab_k_`i'=0 if cachimbo==0
replace dummymax_resp_k_`i'=0 if cachimbo==0
replace dummymin_resp_k_`i'=0 if cachimbo==0
/*
...
*************************************************_k
***********Dummy eco1 neuro `i'
mean Neuro if (dummymax_neuro_k_`i'==1|dummymin_neuro_k_`i'==1) & nombre_curso==238 , over(dummymax_neuro_k_`i') coefl
lincom   _b[c.Neuro@1.dummymax_neuro_k_`i']-_b[c.Neuro@0.dummymax_neuro_k_`i']
matrix eco1`i'[1,6] =  r(estimate)
matrix eco1`i'[2,6] = r(p)
***********Dummy eco1 extrav `i'
mean Extrav if (dummymax_extrav_k_`i'==1|dummymin_extrav_k_`i'==1) & nombre_curso==238 , over(dummymax_extrav_k_`i') coefl
lincom   _b[c.Extrav@1.dummymax_extrav_k_`i']-_b[c.Extrav@0.dummymax_extrav_k_`i']
matrix eco1`i'[1,7] =  r(estimate)
matrix eco1`i'[2,7] = r(p)
***********Dummy eco1 aperex `i'
mean Aperex if (dummymax_aperex_k_`i'==1|dummymin_aperex_k_`i'==1) & nombre_curso==238 , over(dummymax_aperex_k_`i') coefl
lincom   _b[c.Aperex@1.dummymax_aperex_k_`i']-_b[c.Aperex@0.dummymax_aperex_k_`i']
matrix eco1`i'[1,8] =  r(estimate)
matrix eco1`i'[2,8] = r(p)
***********Dummy eco1 amab `i'
mean Amab if (dummymax_amab_k_`i'==1|dummymin_amab_k_`i'==1) & nombre_curso==238 , over(dummymax_amab_k_`i') coefl
lincom   _b[c.Amab@1.dummymax_amab_k_`i']-_b[c.Amab@0.dummymax_amab_k_`i']
matrix eco1`i'[1,9] =  r(estimate)
matrix eco1`i'[2,9] = r(p)
***********Dummy eco1 resp `i'
mean Resp if (dummymax_resp_k_`i'==1|dummymin_resp_k_`i'==1) & nombre_curso==238 , over(dummymax_resp_k_`i') coefl
lincom   _b[c.Resp@1.dummymax_resp_k_`i']-_b[c.Resp@0.dummymax_resp_k_`i']
matrix eco1`i'[1,10] =  r(estimate)
matrix eco1`i'[2,10] = r(p)
*************************************************
tab SECCION_DEL_CURSO if nombre_curso==238 
local a=r(r)+2
putexcel set  "C:\Users\Erzo\OneDrive - Universidad del Pacífico\Univesidad del Pacífico\Ciclo\lCiclo 7 2024 I\Asistente\Big5\Peer effects classrooms\Tablas\eco1`i'", modify
...
*************************************************_k
***********Dummy nivelengua neuro `i'
mean Neuro if (dummymax_neuro_k_`i'==1|dummymin_neuro_k_`i'==1) & nombre_curso==617 , over(dummymax_neuro_k_`i') coefl
lincom   _b[c.Neuro@1.dummymax_neuro_k_`i']-_b[c.Neuro@0.dummymax_neuro_k_`i']
matrix nivelengua`i'[1,6] =  r(estimate)
matrix nivelengua`i'[2,6] = r(p)
***********Dummy nivelengua extrav `i'
mean Extrav if (dummymax_extrav_k_`i'==1|dummymin_extrav_k_`i'==1) & nombre_curso==617 , over(dummymax_extrav_k_`i') coefl
lincom   _b[c.Extrav@1.dummymax_extrav_k_`i']-_b[c.Extrav@0.dummymax_extrav_k_`i']
matrix nivelengua`i'[1,7] =  r(estimate)
matrix nivelengua`i'[2,7] = r(p)
***********Dummy nivelengua aperex `i'
mean Aperex if (dummymax_aperex_k_`i'==1|dummymin_aperex_k_`i'==1) & nombre_curso==617 , over(dummymax_aperex_k_`i') coefl
lincom   _b[c.Aperex@1.dummymax_aperex_k_`i']-_b[c.Aperex@0.dummymax_aperex_k_`i']
matrix nivelengua`i'[1,8] =  r(estimate)
matrix nivelengua`i'[2,8] = r(p)
***********Dummy nivelengua amab `i'
mean Amab if (dummymax_amab_k_`i'==1|dummymin_amab_k_`i'==1) & nombre_curso==617 , over(dummymax_amab_k_`i') coefl
lincom   _b[c.Amab@1.dummymax_amab_k_`i']-_b[c.Amab@0.dummymax_amab_k_`i']
matrix nivelengua`i'[1,9] =  r(estimate)
matrix nivelengua`i'[2,9] = r(p)
***********Dummy nivelengua resp `i'
mean Resp if (dummymax_resp_k_`i'==1|dummymin_resp_k_`i'==1) & nombre_curso==617 , over(dummymax_resp_k_`i') coefl
lincom   _b[c.Resp@1.dummymax_resp_k_`i']-_b[c.Resp@0.dummymax_resp_k_`i']
...
local a=r(r)+2
putexcel set  "C:\Users\Erzo\OneDrive - Universidad del Pacífico\Univesidad del Pacífico\Ciclo\lCiclo 7 2024 I\Asistente\Big5\Peer effects classrooms\Tablas\nivelengua`i'", modify
...
*************************************************_k
***********Dummy nivemate neuro `i'
mean Neuro if (dummymax_neuro_k_`i'==1|dummymin_neuro_k_`i'==1) & nombre_curso==618 , over(dummymax_neuro_k_`i') coefl
lincom   _b[c.Neuro@1.dummymax_neuro_k_`i']-_b[c.Neuro@0.dummymax_neuro_k_`i']
matrix nivemate`i'[1,6] =  r(estimate)
matrix nivemate`i'[2,6] = r(p)
***********Dummy nivemate extrav `i'
mean Extrav if (dummymax_extrav_k_`i'==1|dummymin_extrav_k_`i'==1) & nombre_curso==618 , over(dummymax_extrav_k_`i') coefl
lincom   _b[c.Extrav@1.dummymax_extrav_k_`i']-_b[c.Extrav@0.dummymax_extrav_k_`i']
matrix nivemate`i'[1,7] =  r(estimate)
matrix nivemate`i'[2,7] = r(p)
***********Dummy nivemate aperex `i'
mean Aperex if (dummymax_aperex_k_`i'==1|dummymin_aperex_k_`i'==1) & nombre_curso==618 , over(dummymax_aperex_k_`i') coefl
lincom   _b[c.Aperex@1.dummymax_aperex_k_`i']-_b[c.Aperex@0.dummymax_aperex_k_`i']
matrix nivemate`i'[1,8] =  r(estimate)
matrix nivemate`i'[2,8] = r(p)
***********Dummy nivemate amab `i'
mean Amab if (dummymax_amab_k_`i'==1|dummymin_amab_k_`i'==1) & nombre_curso==618 , over(dummymax_amab_k_`i') coefl
lincom   _b[c.Amab@1.dummymax_amab_k_`i']-_b[c.Amab@0.dummymax_amab_k_`i']
matrix nivemate`i'[1,9] =  r(estimate)
matrix nivemate`i'[2,9] = r(p)
***********Dummy nivemate resp `i'
mean Resp if (dummymax_resp_k_`i'==1|dummymin_resp_k_`i'==1) & nombre_curso==618 , over(dummymax_resp_k_`i') coefl
lincom   _b[c.Resp@1.dummymax_resp_k_`i']-_b[c.Resp@0.dummymax_resp_k_`i']
